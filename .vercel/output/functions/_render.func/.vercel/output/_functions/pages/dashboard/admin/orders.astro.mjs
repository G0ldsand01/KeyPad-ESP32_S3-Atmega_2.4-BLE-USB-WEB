import { c as createComponent, g as renderComponent, r as renderTemplate, d as createAstro, m as maybeRenderHead, e as addAttribute, F as Fragment } from '../../../chunks/astro/server_DYDqy7ws.mjs';
...
import { d as db, u as users, o as orders } from '../../../chunks/index_Dzm_i-4A.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const rows = dbEnabled ? await db.select({
    order: orders,
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": title, "site": true }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="page-content dashboard-page dashboard-page--chrome"> ${renderComponent($$result2, "DashboardChrome", $$DashboardChrome, { "variant": "admin", "title": "Gestion des commandes", "subtitle": dbEnabled ? "Modifier le statut met à jour la base locale." : "Base désactivée (démo Vercel) — commandes non disponibles.", "userName": session.user.name ?? session.user.email ?? "Admin", "userEmail": session.user.email ?? "", "isAdmin": true }, { "default": async ($$result3) => renderTemplate`${rows.length === 0 ? renderTemplate`<p class="dashboard-note">Aucune commande.</p>` : renderTemplate`<div class="admin-orders-wrap" id="admin-orders-root"> <table class="admin-orders-table"> <thead> <tr> <th>Référence</th> <th>Client</th> <th>Total</th> <th>Statut</th> </tr> </thead> <tbody> ${rows.map(({ order: o, customerEmail, customerName }) => {
    const items = parseItems(o.cartJson);
    const qty = items.reduce((n, i) => n + (i.quantity ?? 0), 0);
    return renderTemplate`<tr${addAttribute(o.id, "data-order-id")}> <td> <strong>${o.reference}</strong> <div class="admin-orders-sub"> ${qty} art. ·${" "} ${o.createdAt ? new Date(o.createdAt).toLocaleString("fr-CA") : ""} </div> ${(o.stripePaymentIntentId || o.stripeCheckoutSessionId) && renderTemplate`<div class="admin-orders-sub"> ${o.stripePaymentIntentId ? renderTemplate`${renderComponent($$result3, "Fragment", Fragment, {}, { "default": async ($$result4) => renderTemplate`
PI: <code>${o.stripePaymentIntentId}</code> ` })}` : null} ${o.stripePaymentIntentId && o.stripeCheckoutSessionId ? " · " : ""} ${o.stripeCheckoutSessionId ? renderTemplate`${renderComponent($$result3, "Fragment", Fragment, {}, { "default": async ($$result4) => renderTemplate`
CS: <code>${o.stripeCheckoutSessionId}</code> ` })}` : null} </div>`} </td> <td> ${customerName ?? "—"} <div class="admin-orders-sub">${customerEmail ?? o.userId}</div> </td> <td>${formatCadFromCents(o.totalCents)}</td> <td> <select class="admin-order-status form-control-like"${addAttribute(o.id, "data-order-id")}${addAttribute(`Statut ${o.reference}`, "aria-label")}> <option value="confirmed"${addAttribute(o.status === "confirmed", "selected")}>Confirmée</option> <option value="processing"${addAttribute(o.status === "processing", "selected")}>En traitement</option> <option value="shipped"${addAttribute(o.status === "shipped", "selected")}>Expédiée</option> <option value="cancelled"${addAttribute(o.status === "cancelled", "selected")}>Annulée</option> </select> </td> </tr>`;
  })} </tbody> </table> </div>`}` })} </div> ` })} `;
